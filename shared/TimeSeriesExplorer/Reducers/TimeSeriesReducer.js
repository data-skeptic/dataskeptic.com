...
const defaultState = {
  ...
  query:
    'SELECT COUNT(lat) as val FROM impression where time > now() -2h GROUP BY time(1m)',
  query_results: '',
  plot_data: undefined,
  databases: [],
  measurements: [],
...
function get_databases(dispatch) {
  ...
  axios.get(url).then(function(resp) {
    ...
    dispatch({
      type: 'TSE_UPDATE_DATABASES',
      payload: { databases, dispatch }
    })
  })
}
...
function get_measurements(dispatch) {
  ...
  axios.get(url).then(function(resp) {
    ...
    dispatch({
      type: 'TSE_UPDATE_MEASUREMENTS',
      payload: { measurements, dispatch }
    })
  })
}
