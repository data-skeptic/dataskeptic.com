...
function get_token(nstate, event, dispatch) {
  ...
  Stripe.createToken(event.target, function(status, response) {
    ...
    if (response.error) {
      ...
      nstate.paymentError =
        'We were unable to verify your information.  Please check the information below or contact kyle@dataskeptic.com for support.'
    ...
    dispatch({
      type: 'TOKEN_RECIEVED',
      payload: { dispatch, token, paymentError }
    })
  })
  return nstate
}
...
function token_recieved(dispatch, nstate) {
  ...
  axios
    .post(
      'https://obbec1jy5l.execute-api.us-east-1.amazonaws.com/prod/order',
      order
    )
    .then(function(resp) {
      ...
      setTimeout(function() {
        dispatch({
          ...
          payload: { dispatch, paymentComplete, paymentError }
        })
      }, 10)
    })
    .catch(function(err) {
  ...
  switch (action.type) {
    ...
    case ADD_TO_CART:
      ...
      for (var i = 0; i < nstate.cart_items.length; i++) {
        ...
        if (
          citem.product.id == cart_item.product.id &&
          citem.size == cart_item.size
        ) {
          found_index = i
    ...
    case TOKEN_RECIEVED:
      ...
      if (nstate.paymentError != '') {
        nstate.paymentError =
          'Please make sure you entered your credit card information correctly and try again.  If you continue to have problems, please contact kyle@dataskeptic.com for support.  We apologize for any inconvenience.'
        nstate.submitDisabled = false
        var msg = {
          paymentError: nstate.paymentError,
