...
function handler(dispatch, reply, cstate, message) {
  ...
  if (dt == null) {
    msg =
      "I'm not sure when you mean.  Can you say it another way?  Maybe try 'tomorrow morning'.  Or say 'exit'"
    return { msg, handler }
  }
  console.log(['dt', dt])
...
function sms_handler(dispatch, reply, cstate, message) {
  ...
  var msg =
    "That phone number seems to be invalid.  Please try again or say 'exit'"
...
function save_handler(dispatch, reply, cstate, message) {
  ...
  axios
    ...
    .catch(function(err) {
      reply(
        { text: "Darn, I wasn't able to save that.  Sorry!  You found a bug!" },
        'bot'
      )
    })
