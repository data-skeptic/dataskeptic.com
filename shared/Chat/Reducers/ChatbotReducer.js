...
export default function ChatbotReducer(state = defaultState, action) {
  ...
  switch (action.type) {
    ...
    case 'ASK_QUESTION':
      ...
      nstate.thinking = true
      var promise = get_question(question_id)
        .then(function(result) {
    ...
    case 'BOT_FAILURE':
      ...
      var reply = action.reply
      var msg =
        "Oh my.  I don't feel so well.  Could we not talk about this?  It hurts when the conversation comes to this point.  It's quite unpleasant to have an error_code " +
        error_code +
        '.  Be a pal and change the subject, ok?'
    ...
    case 'SURVEY_COMPLETE':
      action.reply(
        { text: "That's the end of the survey.  Thank you for your input!" },
        'bot'
      )
      ...
      axios
        ...
        .then(function(result) {
          ...
          if (blogs.length > 0) {
            ...
            if (blog) {
              var message = { blog }
              resp = episodes.make_recommendation_handler(
                dispatch,
                reply,
                nstate,
                message
